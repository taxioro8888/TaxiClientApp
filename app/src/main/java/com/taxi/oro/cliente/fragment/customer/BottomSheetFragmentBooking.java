...
import com.taxi.oro.cliente.adapter.CategoryVehicleAdapter;
import com.taxi.oro.cliente.controller.AppController;
import com.taxi.oro.cliente.directionhelpers.FetchURL;
import com.taxi.oro.cliente.model.CategoryVehiclePojo;
                            ...
                            try {
                                ...
                                if(etat.equals("1")){
                                    for(int i=0; i<(msg.length()-1); i++) {
                                        JSONObject cliente = msg.getJSONObject(String.valueOf(i));
                                        albumList_driver.add(new DriverPojo(cliente.getInt("id"),cliente.getInt("idConducteur"),cliente.getString("nom")+" "+cliente.getString("prenom"),
                                                cliente.getString("immatriculation"), cliente.getString("numero")));
                                        adapter_driver.notifyDataSetChanged();
                                    }
                                }else{
